    ...
    switch (a) {
        case "menu": switch (b) {
            ...
            case "2": a = get_matbyid(d); b = ""; 1 == a.matinfo.isadd && (b = '<a class="button" target="_blank" href="' + a.matinfo.add2 + '">' + a.matinfo.add1 + "</a>");
                //openModal("html", a.matinfo.contents + b, a.matinfo.title);
           
                if (a.matinfo.add2 == "pdf") {
                    console.log(a.matinfo)
                    //window.location.href = _config.app_url + datas.matinfo.mat_file;
                    openModal('iframe', _config.app_url + a.matinfo.mat_file, a.matinfo.title, 'full');
                }
                else {
                    openModal('html', a.matinfo.contents, a.matinfo.title, 'full');
                }
                break;
            ...
            case "8": openModal2("html", '<div class="player-audio" style="text-align:center"><audio controls="controls" autoplay="autoplay"><source type="audio/mpeg" src="' + d + '"></source>\u60a8\u7684\u6d4f\u89c8\u5668\u4e0d\u652f\u6301HTML5\u64ad\u653e\u97f3\u9891</audio></div>', c, "small"); break;

            case '9':
                _krpano.call("stopsound(bgmusic);");
                console.log(d);
                if (d.indexOf('*') == -1) {
                    var html = '<div class="player-audio" style="text-align:center;max-height:520px;width: 800px;"><video controls="controls" autoplay="autoplay" style="width:auto;height:100%;"><source type="video/mp4" src="/' + d + '"></source>您的浏览器不支持HTML5播放视频</video></div>';
                    if (_config.client == "pc")
                        openModal2('html', html, c, 'content');
                    else {
...
function openModal2(a, b, c, d) {
    switch (a) {
        case "iframe": $("body").createModal({ iframe: b, title: c, size: d });
            break;
        case "html": $("body").createModal({ html: b, title: c, size: d });
            break;
        case "msg": $("body").createModal({ msg: b, bgClose: !1 })
    }
}
    ...
    switch (type) {
        ...
        case 'html':
            ...
            var layindex = layer.open({
                ...
                area: ['auto', '80%'],
                content: '<div class="layui-layer-content-custom">' + data + '</div>'
            });
            break;
        case 'atuohtml':
            layer.open({
...
krpanoplugin.prototype.addviewhotspot = function (hotspotdata) {
    ...
    if (hotspotdata.isstatic == 1) {//固定平面热点
        ...
        _krpano.call("set(layer[" + hotspotName + "].x," + (gnindex * 50 + 130) + ");");
        _krpano.call("set(layer[" + hotspotName + "].width,40);");
        _krpano.call("set(layer[" + hotspotName + "].height,40);");
        _krpano.call("set(layer[" + hotspotName + "].align,'rightbottom');");
        _krpano.call("set(layer[" + hotspotName + "].y,20);");
        ...
        if (hotspotdata.hot_name) {
            ...
            if (hotspotdata.diy_src.indexOf('g-8') == -1 && hotspotdata.diy_src.indexOf('g-9') == -1 && hotspotdata.diy_src.indexOf('g-10') == -1) {
                ...
                var txtlen = hotspotdata.hot_name.length;
                var paddingtop = 110;

                if (hotspotdata.diy_src.indexOf('g-8') != -1) {
                    paddingtop = 110 + (5 - txtlen) * 10;
                }
                else if (hotspotdata.diy_src.indexOf('g-9') != -1) {
                    paddingtop = 100 + (5 - txtlen) * 10;
                }
                else {
                    paddingtop = 110 + (6 - txtlen) * 10;
                }
                this.addlayer(textName, {
                    parent: hotspot,
                    url: "%SWFPATH%/plugins/textfield.swf",
...
krpanoplugin.prototype.clcikhotspot = function (a) {
    a = _krpano.hotspots[_krpano.scene_id][a]; switch (a.type_id) {
        ...
        case '9':
            _krpano.call("stopsound(bgmusic);");
            var html = '<div class="player-audio" style="text-align:center;padding:20px 20px;width: 800px;"><video controls="controls" autoplay="autoplay" style="width:100%;"><source type="video/mp4" src="' + a.v + '"></source>您的浏览器不支持HTML5播放视频</video></div>';
...
function initNavMenu() {
    //var logourl = _krpano.get('config.logo_url')
    //if (logourl && logourl != '') {
    //    //$("#returnhome").attr("href", logourl);
    //    $("#returnhome").show();
    //}
    $("#returnhome").click(function () {
        _krpano.call('skin_loadscene(get(scene[0].name));');
    })
    $("#returnhome").show();
    var data = getnavmenu();

    if (data.menulist.length > 0) {
        $(".nav-user").append('<a id="showhidenav" style="position:absolute;right:-40px;top:28px;cursor: pointer;"><img src="/krpano/images/menu.png" /></a>');
        $("#showhidenav").on('click', function (ev) {
            if (navshow) $(".primary").hide();
            else $(".primary").show();
            navshow = !navshow;
        })
    }
    ...
    for (var m = data.menulist.length - 1; m >= 0 ; m--) {
        if (data.menulist[m].parent_id == 0) {
            ...
            var pcchild = '<li class="level-0"><a data-id="' + data.menulist[m].menu_id + '"><span>' + data.menulist[m].title.charAt(0) + '</span>' + data.menulist[m].title.substring(1) + '</a>' + (isexist == true ? '' : '');
                ...
                for (var n = 0; n < data.menulist.length; n++) {
                    if (data.menulist[n].parent_id != 0) {
                        if (data.menulist[m].menu_id == data.menulist[n].parent_id) {
                            ...
                            pcchild += '<li class="level-1 active"  data-id="' + data.menulist[n].menu_id + '"><a style="width:auto;min-width:160px; padding-left:0px;padding-right:0px;font-size:12px;">' + data.menulist[n].title + '</a></li>';
                            mobilechild += '<li class="sub-menu-item" data-id="' + data.menulist[n].menu_id + '"><a style="width:auto;min-width:160px; padding-left:0px;padding-right:0px;font-size:12px;">' + data.menulist[n].title + '</a></li>';
                        }
                    }
                }
                pcchild += '</ol>';
